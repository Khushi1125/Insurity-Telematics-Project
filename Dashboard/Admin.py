...
def show_dashboard():
    ...
    if selected_graph == "Night Driving Fraction":
        ...
        # Draw purple filled portion according to percentage
        ax.barh([1], [night_pct], height=0.4, color="#7a6de8")
        ax.set_xlim(0, 1)
        ax.axis('off')  # no ticks or labels
        st.pyplot(fig)
        # Show percent below
        ...
        if not driver_telemetry.empty and "timestamp" in driver_telemetry.columns:
            df = driver_telemetry.copy()
            df['harsh_accel'] = df['acceleration'] > 4  # threshold for harsh accel
            df['timestamp'] = pd.to_datetime(df['timestamp'])
